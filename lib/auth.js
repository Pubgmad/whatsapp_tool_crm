...
export async function currentAccount(request) {
  ...
  const result = await query(
    `SELECT u.id AS user_id, u.name AS user_name, u.email,
            b.id AS business_id, b.name AS business_name, b.account_status,
            COALESCE(bs.status, 'pending') AS subscription_status,
            COALESCE(sp.name, 'Unassigned') AS subscription_plan,
            m.role
     FROM memberships m
...
export async function registerAccount({ name, email, password, businessName }) {
  ...
  return transaction(async (client) => {
    ...
    const plan = defaultPlanCode
      ? await client.query("SELECT id, trial_days FROM subscription_plans WHERE code = $1 AND is_active = TRUE LIMIT 1", [defaultPlanCode])
      : { rows: [] };
    await client.query(
