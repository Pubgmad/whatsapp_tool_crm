...
export async function loginAccount({ email, password, mfaCode, businessId }) {
  const cleanEmail = normalizeEmail(email);
  const user = await query('SELECT id,password_hash,email_verified_at,mfa_enabled,mfa_secret_encrypted,mfa_recovery_codes,session_version FROM users WHERE email=$1', [cleanEmail]);
  const row = user.rows[0];
  if (!row || !verifyPassword(password, row.password_hash)) {
    throw new AppError("Invalid email or password.", 401, "INVALID_CREDENTIALS");
  }
  if (process.env.EMAIL_VERIFICATION_REQUIRED === 'true' && !row.email_verified_at) throw new AppError('Verify your email before signing in.', 403, 'EMAIL_NOT_VERIFIED');
  if (row.mfa_enabled) {
