...
export async function authenticateSessionToken(token) {
  ...
  const { enterTenantContext } = await import('./db');
...
export async function loginAccount({ email, password, mfaCode }) {
  const cleanEmail = normalizeEmail(email);
  const user = await query('SELECT id,password_hash,email_verified_at,mfa_enabled,mfa_secret_encrypted,mfa_recovery_codes FROM users WHERE email=$1', [cleanEmail]);
  ...
  if (row.mfa_enabled) {
    ...
    const { verifyMfaForUser } = await import('./account-security');
