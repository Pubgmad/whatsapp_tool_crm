...
import { secureCookieAttribute } from "./security.js";
...
export function emailVerificationRequired() {
  return process.env.NODE_ENV === "production" || process.env.EMAIL_VERIFICATION_REQUIRED === "true";
}

export function assertRegistrationEmailReady() {
  if (!emailVerificationRequired() || process.env.NODE_ENV !== "production") return;
  if (!process.env.RESEND_API_KEY?.trim() || !process.env.EMAIL_FROM?.trim() || !process.env.APP_URL?.trim()) {
    throw new AppError("Email verification must be configured before registration is available.", 503, "EMAIL_NOT_CONFIGURED");
  }
  let url;
  try { url = new URL(process.env.APP_URL); }
  catch { throw new AppError("APP_URL must be a valid HTTPS URL.", 503, "APP_URL_HTTPS_REQUIRED"); }
  if (url.protocol !== "https:") {
    throw new AppError("APP_URL must use HTTPS for production registration.", 503, "APP_URL_HTTPS_REQUIRED");
  }
}
...
function validSignature(value, signature) {
  return [process.env.AUTH_SECRET, process.env.AUTH_SECRET_PREVIOUS].filter(Boolean).some((key) => {
...
export function sessionCookie(token) {
  return `${SESSION_COOKIE_NAME}=${encodeURIComponent(token)}; Path=/; HttpOnly; SameSite=Lax; Max-Age=${SESSION_SECONDS}${secureCookieAttribute()}`;
}
...
export async function authenticateSessionToken(token) {
  ...
  const { enterTenantContext } = await import('./db.js');
  enterTenantContext(session.businessId);

  const result = await query(
    `SELECT m.role, b.account_status, u.session_version, u.email_verified_at, u.email_verification_required
     FROM memberships m
  ...
  if (Number(session.sessionVersion || 0) !== Number(membership.session_version)) {
    throw new AppError("This session has expired. Sign in again.", 401, "SESSION_REVOKED");
  }
  if ((membership.email_verification_required || process.env.EMAIL_VERIFICATION_REQUIRED === 'true') && !membership.email_verified_at) {
...
export async function requireSession(request) {
  const { assertCsrf, enforceRequestRateLimit } = await import('./security.js');
  assertCsrf(request);
  ...
  const session = await authenticateSessionToken(parseCookies(request)[SESSION_COOKIE_NAME]);
  await enforceRequestRateLimit(request, session.userId, 'api');
  return session;
}
...
export async function registerAccount({ name, email, password, businessName }) {
  ...
  return transaction(async (client) => {
    ...
    await client.query('SELECT set_config(\'app.business_id\',$1,true),set_config(\'app.system_access\',\'false\',true)', [businessId]);
    const membershipId = id("mb");
    const baseSlug = slugify(cleanBusinessName);
    const slug = `${baseSlug}-${crypto.randomBytes(2).toString("hex")}`;

    await client.query("INSERT INTO users (id, name, email, password_hash, email_verification_required) VALUES ($1, $2, $3, $4, $5)", [userId, cleanName, cleanEmail, hashPassword(password), emailVerificationRequired()]);
    await client.query(
...
export async function loginAccount({ email, password, mfaCode, businessId }) {
  const cleanEmail = normalizeEmail(email);
  const user = await query('SELECT id,password_hash,email_verified_at,email_verification_required,mfa_enabled,mfa_secret_encrypted,mfa_recovery_codes,session_version FROM users WHERE email=$1', [cleanEmail]);
  const row = user.rows[0];
  if (!row || !verifyPassword(password, row.password_hash)) {
    throw new AppError("Invalid email or password.", 401, "INVALID_CREDENTIALS");
  }
  if ((row.email_verification_required || process.env.EMAIL_VERIFICATION_REQUIRED === 'true') && !row.email_verified_at) throw new AppError('Verify your email before signing in.', 403, 'EMAIL_NOT_VERIFIED');
  if (row.mfa_enabled) {
    if (!mfaCode) throw new AppError('Enter your authenticator or recovery code.', 401, 'MFA_REQUIRED');
    const { verifyMfaForUser } = await import('./account-security.js');
    if (!await verifyMfaForUser(row, mfaCode, row.id)) throw new AppError('The authentication code is invalid.', 401, 'MFA_INVALID');
  }
  const { enterSystemContext } = await import("./db.js");
  enterSystemContext();
  const requestedBusinessId = String(businessId || "").trim();
  const membership = await query(
    `SELECT m.business_id, m.role, b.account_status
       FROM memberships m JOIN businesses b ON b.id = m.business_id
      WHERE m.user_id = $1 AND ($2 = '' OR m.business_id = $2)
      ORDER BY CASE WHEN b.account_status = 'suspended' THEN 1 ELSE 0 END, m.created_at ASC
      LIMIT 1`,
    [row.id, requestedBusinessId]
  );
  const member = membership.rows[0];
  if (!member) throw new AppError("No business workspace is linked to this account.", 403, "NO_WORKSPACE");
  if (member.account_status === "suspended") throw new AppError("This company workspace is suspended.", 403, "ACCOUNT_SUSPENDED");
  return { userId: row.id, businessId: member.business_id, role: member.role, sessionVersion: row.session_version };
}
