...
function providerConfig() {
  ...
  if (!/^\d+$/.test(appId) || !/^\d+$/.test(configId)) throw new AppError('Meta App ID or Embedded Signup configuration ID is invalid.', 503, 'META_EMBEDDED_SIGNUP_INVALID');
  const appUrl = clean(process.env.APP_URL);
  if (process.env.NODE_ENV === 'production' && (!appUrl || new URL(appUrl).protocol !== 'https:')) throw new AppError('APP_URL must be the public HTTPS application URL for Meta Embedded Signup.', 503, 'META_HTTPS_REQUIRED');
...
function requireConnectionManager(session) {
  if (!["Owner", "Manager"].includes(session.role)) {
    throw new AppError("Only workspace owners and managers can change the WhatsApp connection.", 403, "META_CONNECTION_FORBIDDEN");
  }
}
...
export async function verifyManualWhatsAppCredentials({ wabaId, phoneNumberId, accessToken }) {
  const appId = clean(process.env.META_APP_ID);
  const appSecret = clean(process.env.META_APP_SECRET);
  if (!appId || !appSecret) throw new AppError("Meta App ID and App Secret must be configured on the server.", 503, "META_APP_NOT_CONFIGURED");
  if (!/^\d+$/.test(wabaId) || !/^\d+$/.test(phoneNumberId) || !accessToken) {
    throw new AppError("A valid WABA ID, phone number ID, and access token are required.", 400, "META_CREDENTIALS_REQUIRED");
  }
  const tokenData = await debugToken(accessToken, { appId, appSecret });
  const numbers = await graphRequest(`${encodeURIComponent(wabaId)}/phone_numbers?fields=id,display_phone_number,verified_name,status&limit=100`, accessToken);
  const phone = (numbers.data || []).find((item) => String(item.id) === phoneNumberId);
  if (!phone) throw new AppError("The phone number is not accessible under this WABA with the supplied token.", 403, "META_PHONE_NOT_ACCESSIBLE");
  await graphRequest(`${encodeURIComponent(wabaId)}/subscribed_apps`, accessToken, { method: "POST", body: "{}" });
  const expiresAt = Number(tokenData.expires_at || 0) > 0 ? new Date(Number(tokenData.expires_at) * 1000) : null;
  return { phone, expiresAt, scopes: tokenData.scopes || [] };
}

export async function getEmbeddedSignupConfig(request) {
  try {
    const account = await currentAccount(request);
    requireConnectionManager(account);
    const { appId, configId } = providerConfig();
    return json({ appId, configId, graphVersion: graphVersion(), appOrigin: new URL(process.env.APP_URL || request.url).origin, enabled: true });
...
export async function completeEmbeddedSignup(request) {
  try {
    ...
    requireConnectionManager(session);
    ...
    const exchangeResponse = await fetch(graphUrl("oauth/access_token"), { method: "POST", headers: { "Content-Type": "application/x-www-form-urlencoded" }, body: form, cache: "no-store", redirect: "error", signal: AbortSignal.timeout(30000) });
    ...
    await transaction(async (client) => {
      ...
      const accountRow = await client.query(
        ...
        [id("waa"), session.businessId, wabaId, encryptSecret(exchange.access_token), expiresAt,
          JSON.stringify({ cloud_api: true, webhooks: true, templates: true, interactive_messages: true }),
...
export async function checkMetaConnection(request) {
  try {
    ...
    const subscribed = (subscriptions.data || []).some((item) => String(item.whatsapp_business_api_data?.id || item.id) === config.appId);
    const expiresAt = Number(debug.expires_at || 0) > 0 ? new Date(Number(debug.expires_at) * 1000) : null;
    await transaction(async (client) => {
...
export async function disconnectMeta(request) {
  try {
    ...
    const accounts = (await query(
      "SELECT waba_id, access_token_encrypted, webhook_subscribed FROM whatsapp_accounts WHERE business_id = $1",
      [session.businessId]
    )).rows;
    const unsubscribed = new Set();
    for (const account of accounts) {
      if (!account.webhook_subscribed) continue;
      if (!account.access_token_encrypted) throw new AppError("WhatsApp access token is missing. Resolve the Meta connection before disconnecting.", 409, "META_TOKEN_MISSING");
      await graphRequest(`${encodeURIComponent(account.waba_id)}/subscribed_apps`, decryptSecret(account.access_token_encrypted), { method: "DELETE" });
      unsubscribed.add(account.waba_id);
      await query("UPDATE whatsapp_accounts SET webhook_subscribed = FALSE, updated_at = NOW() WHERE business_id = $1 AND waba_id = $2", [session.businessId, account.waba_id]);
      if (business?.waba_id === account.waba_id) {
        await query("UPDATE businesses SET webhook_subscribed = FALSE, updated_at = NOW() WHERE id = $1", [session.businessId]);
      }
    }
    if (business?.webhook_subscribed && business.waba_id && !unsubscribed.has(business.waba_id)) {
      if (!business.access_token_encrypted) throw new AppError("WhatsApp access token is missing. Resolve the Meta connection before disconnecting.", 409, "META_TOKEN_MISSING");
      await graphRequest(`${encodeURIComponent(business.waba_id)}/subscribed_apps`, decryptSecret(business.access_token_encrypted), { method: "DELETE" });
      await query("UPDATE businesses SET webhook_subscribed = FALSE, updated_at = NOW() WHERE id = $1", [session.businessId]);
    }
    await transaction(async (client) => {
