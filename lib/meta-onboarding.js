...
export async function checkMetaConnection(request) {
  try {
    ...
    await query("UPDATE businesses SET webhook_subscribed = $1, meta_token_expires_at = $2, updated_at = NOW() WHERE id = $3", [subscribed, Number(debug.expires_at || 0) > 0 ? new Date(Number(debug.expires_at) * 1000) : null, session.businessId]);
...
export async function disconnectMeta(request) {
  try {
    ...
    await query(`UPDATE businesses SET whatsapp_number = '', waba_id = '', phone_number_id = '', access_token_encrypted = '', status = 'Needs setup', onboarding_method = 'manual', meta_token_expires_at = NULL, meta_connected_at = NULL, webhook_subscribed = FALSE, meta_connection_metadata = '{}'::jsonb, updated_at = NOW() WHERE id = $1`, [session.businessId]);
    await query("INSERT INTO audit_logs (id, business_id, user_id, action) VALUES ($1, $2, $3, 'meta_disconnected')", [id("a"), session.businessId, session.userId]);
    return json({ ok: true });
  } catch (error) { return errorJson(error); }
}