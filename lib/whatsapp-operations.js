...
import { assertMessageCapacity } from "./limits";
import { readJsonBodyLimited, readMultipartFormLimited } from "./security";
...
async function graphRequest(path, token, options = {}) {
  const response = await fetch(graphUrl(path), {
    ...
    cache: "no-store",
    redirect: "error",
    signal: options.signal || AbortSignal.timeout(options.body instanceof FormData ? 120000 : 30000)
  });
...
function capabilityState(account, phones, flows, templates) {
  ...
  const accountPhones = phones.filter((item) => item.whatsapp_account_id === account?.id);
  const activePhone = accountPhones.find((item) => item.is_default) || accountPhones[0];
  const tokenValid = Boolean(account?.access_token_encrypted) &&
    (!account.token_expires_at || new Date(account.token_expires_at).getTime() > Date.now());
  const accountReady = account?.status === "connected" && tokenValid;
  const phoneReady = activePhone?.registration_state === "registered";
  const inferred = {
    cloud_api: Boolean(accountReady && phoneReady),
    webhooks: Boolean(accountReady && account.webhook_subscribed),
    templates: Boolean(accountReady),
    interactive_messages: Boolean(accountReady && phoneReady),
    ...
    ctwa: false,
    marketing_messages_api: false,
    calling: false,
    billing_visibility: false
  };
  return CAPABILITY_DEFINITIONS.map((item) => ({
    ...item,
    status: inferred[item.key] ? "available" : ["cloud_api", "webhooks", "templates", "interactive_messages", "native_flows", "authentication_templates", "media_templates"].includes(item.key) ? "setup" : "not_integrated",
...
function mapPhone(row) {
  return {
    ...
    flowEncryption: { configured: Boolean(row.flow_private_key_encrypted), signatureStatus: row.flow_key_signature_status || "" },
    lastSyncedAt: toIso(row.last_synced_at)
  };
}
...
function mapFlow(row) {
  return {
    ...
    managedEndpoint: Boolean(row.endpoint_phone_id),
...
async function sendCommerceMessage(businessId, input) {
  ...
  await assertMessageCapacity(businessId, 1, null, contact.id);
  ...
  if (!metaMessageId) throw new AppError("Meta did not confirm the product message ID. Check delivery before resending.", 409, "META_SEND_UNCONFIRMED");
  await transaction(async (client) => {
    const conversation = (await client.query(
      `INSERT INTO conversations (id,business_id,contact_id,updated_at,whatsapp_phone_number_id)
       VALUES ($1,$2,$3,NOW(),$4)
       ON CONFLICT (business_id,contact_id) DO UPDATE SET updated_at=NOW(),whatsapp_phone_number_id=EXCLUDED.whatsapp_phone_number_id
       RETURNING id`,
      [id("v"), businessId, contact.id, phone.phone_number_id]
    )).rows[0];
    await client.query(
      `INSERT INTO messages (id,conversation_id,direction,body,status,meta_message_id,message_type,metadata)
       VALUES ($1,$2,'outgoing',$3,'sent',$4,'interactive',$5) ON CONFLICT DO NOTHING`,
      [id("m"), conversation.id, clean(input.body), metaMessageId, JSON.stringify({ catalogId, productCount: products.length })]
    );
    await client.query("INSERT INTO events (id,business_id,type,metadata) VALUES ($1,$2,'commerce_message_sent',$3)", [id("e"), businessId, JSON.stringify({ contactId: contact.id, phoneNumberId: phone.phone_number_id, metaMessageId, catalogId, productCount: products.length })]);
  });
}
  ...
  for (const item of payload.data || []) {
    await query(
      `INSERT INTO whatsapp_native_flows (id,business_id,whatsapp_account_id,meta_flow_id,name,category,status,endpoint_uri,validation_errors)
       ...
       endpoint_uri=COALESCE(NULLIF(EXCLUDED.endpoint_uri,''),whatsapp_native_flows.endpoint_uri),
       validation_errors=EXCLUDED.validation_errors,updated_at=NOW()`,
...
async function createFlow(businessId, input) {
  ...
  const flowId = id("waf");
  let endpointUri = clean(input.endpointUri);
  let endpointPhoneId = null;
  if (input.managedEndpoint) {
    const { phone } = await phoneForBusiness(businessId, input.phoneId);
    if (phone.whatsapp_account_id !== account.id) throw new AppError("Flow and phone must belong to the same WhatsApp account.", 400, "FLOW_ACCOUNT_MISMATCH");
    if (!phone.flow_private_key_encrypted || phone.flow_key_signature_status !== "VALID") throw new AppError("Configure and verify Flow encryption for this number first.", 400, "FLOW_ENCRYPTION_REQUIRED");
    if (!process.env.APP_URL) throw new AppError("APP_URL is required for a managed Flow endpoint.", 503, "APP_URL_REQUIRED");
    const appUrl = new URL(process.env.APP_URL);
    if (appUrl.protocol !== "https:" && process.env.NODE_ENV === "production") throw new AppError("APP_URL must use HTTPS for Meta Flows.", 503, "APP_URL_HTTPS_REQUIRED");
    endpointUri = new URL(`/api/whatsapp/flows/data/${flowId}`, appUrl).toString();
    endpointPhoneId = phone.id;
  }
  const existing = await query("SELECT id FROM whatsapp_native_flows WHERE business_id=$1 AND name=$2", [businessId, name]);
  if (existing.rowCount) throw new AppError("A Flow with this name already exists.", 409, "FLOW_EXISTS");
  await query(
    `INSERT INTO whatsapp_native_flows (id,business_id,whatsapp_account_id,meta_flow_id,name,category,status,endpoint_uri,endpoint_phone_id)
     VALUES ($1,$2,$3,$4,$5,$6,'draft',$7,$8)`,
    [flowId, businessId, account.id, "", name, category, endpointUri, endpointPhoneId]
  );
  let createdMetaId = "";
  try {
    ...
    if (!created.id) throw new AppError("Meta did not return a Flow ID. Check Meta before retrying.", 409, "FLOW_CREATE_UNCONFIRMED");
    createdMetaId = String(created.id);
    await query("UPDATE whatsapp_native_flows SET meta_flow_id=$1,updated_at=NOW() WHERE id=$2 AND business_id=$3", [createdMetaId, flowId, businessId]);
  } catch (error) {
    if (!createdMetaId && error.code !== "FLOW_CREATE_UNCONFIRMED") await query("DELETE FROM whatsapp_native_flows WHERE id=$1 AND business_id=$2 AND meta_flow_id=''", [flowId, businessId]);
    throw error;
  }
}

async function configureFlowEncryption(businessId, phoneId) {
  const { phone, token } = await phoneForBusiness(businessId, phoneId);
  let privateKey = phone.flow_private_key_encrypted ? decryptSecret(phone.flow_private_key_encrypted) : "";
  let publicKey = phone.flow_public_key || "";
  if (!privateKey || !publicKey) {
    ...
    await query("UPDATE whatsapp_phone_numbers SET flow_private_key_encrypted=$1,flow_public_key=$2,flow_key_signature_status='',updated_at=NOW() WHERE id=$3 AND business_id=$4", [encryptSecret(privateKey), publicKey, phone.id, businessId]);
  }
  const form = new FormData();
  form.set("business_public_key", publicKey);
  await graphRequest(`${encodeURIComponent(phone.phone_number_id)}/whatsapp_business_encryption`, token, { method: "POST", body: form });
  const verified = await graphRequest(`${encodeURIComponent(phone.phone_number_id)}/whatsapp_business_encryption`, token);
  const matching = (verified.data || []).find((item) => item.business_public_key?.replace(/\s/g, "") === publicKey.replace(/\s/g, ""));
  const status = clean(matching?.business_public_key_signature_status);
  await query("UPDATE whatsapp_phone_numbers SET flow_key_signature_status=$1,updated_at=NOW() WHERE id=$2 AND business_id=$3", [status, phone.id, businessId]);
}

async function setFlowResponses(businessId, flowId, responses) {
  const flow = (await query("SELECT id,endpoint_phone_id FROM whatsapp_native_flows WHERE id=$1 AND business_id=$2", [clean(flowId), businessId])).rows[0];
  if (!flow?.endpoint_phone_id) throw new AppError("This Flow does not use the managed data endpoint.", 400, "FLOW_ENDPOINT_NOT_MANAGED");
  if (!responses || typeof responses !== "object" || Array.isArray(responses) || JSON.stringify(responses).length > 32768) {
    throw new AppError("Flow responses must be a JSON object under 32 KB.", 400, "INVALID_FLOW_RESPONSES");
  }
  for (const [key, value] of Object.entries(responses)) {
    if (!/^(INIT|(?:data_exchange|navigate|BACK):[A-Za-z0-9_]+)$/.test(key) ||
        !value || typeof value !== "object" || Array.isArray(value) ||
        typeof value.screen !== "string" || !value.screen ||
        !value.data || typeof value.data !== "object" || Array.isArray(value.data)) {
      throw new AppError("Each response needs a valid action:screen key, screen, and data object.", 400, "INVALID_FLOW_RESPONSES");
    }
  }
  await query("UPDATE whatsapp_native_flows SET endpoint_responses=$1,updated_at=NOW() WHERE id=$2 AND business_id=$3", [JSON.stringify(responses), flow.id, businessId]);
}
...
async function publishFlow(businessId, flowId) {
  ...
  if (flow.endpoint_phone_id && !flow.endpoint_responses?.INIT) {
    throw new AppError("Configure the INIT data response before publishing this Flow.", 400, "FLOW_RESPONSE_REQUIRED");
  }
  const { token } = await accountForBusiness(businessId, flow.whatsapp_account_id);
...
export async function updateWhatsAppOperations(request) {
  try {
    ...
    else if (action === "configure_flow_encryption") await configureFlowEncryption(session.businessId, body.phoneId);
    else if (action === "set_flow_responses") await setFlowResponses(session.businessId, body.flowId, body.responses);
...
export async function uploadWhatsAppMedia(request) {
  try {
    const session = await requireSession(request);
    assertManager(session);
    const form = await readMultipartFormLimited(request, 101 * 1024 * 1024);
