...
function capabilityState(account, phones) {
  ...
  const inferred = {
    ...
    native_flows: Boolean(account),
    authentication_templates: Boolean(account),
    media_templates: Boolean(account),
...
export async function getWhatsAppOperationsState(businessId) {
  const [accountsResult, phonesResult, flowsResult, analyticsResult] = await Promise.all([
  ...
  return {
    ...
    capabilities: capabilityState(account, phonesResult.rows),
