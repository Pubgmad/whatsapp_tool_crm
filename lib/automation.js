...
import { metaReady, sendInteractiveMessage, sendTextMessage } from "./meta";
...
export async function runAutomationQueue({ businessId = "", limit = batchSize() } = {}) {
  ...
  const summary = { claimed: jobs.length, sent: 0, completed: 0, handoff: 0, failed: 0, retried: 0 };
  ...
  for (const job of jobs) {
    try {
      ...
      await query("UPDATE automation_sessions SET last_error = $1, updated_at = NOW() WHERE id = $2", [clean(error.message), job.session_id]);
  ...
  return transaction(async (client) => {
    ...
    const result = await client.query(
      `SELECT j.id AS job_id, j.session_id, j.attempts, j.max_attempts, j.input,
              s.current_node_id, s.context, s.contact_id,
...
async function processJob(job, summary) {
  ...
  if (!insideReplyWindow(job.last_message_at)) throw new AppError("Automation cannot send free-form messages outside the 24-hour WhatsApp reply window.", 403, "REPLY_WINDOW_CLOSED");

  const definition = normalizeDefinition(job.definition);
  const input = job.input || {};
  const currentNode = getNode(definition, job.current_node_id);
  ...
  await query(
    `UPDATE automation_sessions
     SET current_node_id = $1, context = $2, status = $3, human_takeover = $4,
         last_input = $5, last_error = '', updated_at = NOW(), ended_at = CASE WHEN $3 IN ('handoff', 'completed') THEN NOW() ELSE ended_at END
     WHERE id = $6`,
    [targetNode.id, JSON.stringify(context), status, status === "handoff", clean(input.text || input.value || input.title), job.session_id]
  );
...
function normalizeNode(node) {
  ...
  const type = ["question", "message", "handoff", "end"].includes(node.type) ? node.type : "question";
...
function mapFlow(row) {
  ...
  return {
    ...
    handoffSessions: row.handoff_sessions || 0,
