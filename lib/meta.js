...
export async function createWhatsAppTemplate({ setup, name, body, category = "MARKETING", language = "en_US", headerText = "", footerText = "", buttons = [], componentSchema = {} }) {
  ...
  const components = isAuthentication
    ? [
        ...
        { type: "BUTTONS", buttons: [{ type: "OTP", otp_type: String(componentSchema.otpType || "COPY_CODE").toUpperCase(), text: String(componentSchema.otpButtonText || "Copy code").slice(0, 25) }] }
      ]
    : [
