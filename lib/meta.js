...
export async function createWhatsAppTemplate({ setup, name, body, category = "MARKETING", language = "en_US", headerText = "", footerText = "", buttons = [] }) {
  ...
  const response = await fetch(graphUrl(`${setup.waba_id}/message_templates`), {
    ...
    body: JSON.stringify({
      ...
      category,
components: [
        ...(headerText ? [{ type: "HEADER", format: "TEXT", text: String(headerText) }] : []),
        { type: "BODY", text: body },
        ...(footerText ? [{ type: "FOOTER", text: String(footerText) }] : []),
        ...(buttons.length ? [{ type: "BUTTONS", buttons: buttons.slice(0, 3).map((button) => ({ type: "QUICK_REPLY", text: String(button.text).slice(0, 25) })) }] : [])
      ]
    })
  });
...
export async function sendInteractiveMessage({ setup, to, body, options = [], mode = "buttons", buttonText = "Choose", sectionTitle = "Options" }) {
    ...
    : {
        ...
        action: {
          ...
          sections: [{
            ...
            rows: cleanOptions.slice(0, 10).map((option) => ({
              id: option.id,
              title: option.title,
  ...
  const payload = await response.json().catch(() => ({}));
  if (!response.ok) {
    throw new AppError(payload.error?.message || "Meta interactive message send failed.", response.status, "META_SEND_FAILED");
  }
  return { status: "sent", metaMessageId: payload.messages?.[0]?.id || id("wamid"), provider: "meta" };
}

export async function sendTemplateMessage({ setup, to, templateName, language = "en_US", variables = [] }) {
  const token = accessToken(setup);
  const response = await fetch(graphUrl(`${setup.phone_number_id}/messages`), {
    method: "POST",
    headers: {
...
export async function fetchWhatsAppMedia({ setup, mediaId }) {
  const token = accessToken(setup);
  const metadataResponse = await fetch(graphUrl(encodeURIComponent(String(mediaId || ""))), {
    headers: { Authorization: `Bearer ${token}` },
    cache: "no-store"
  });
