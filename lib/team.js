...
import { AppError, enterSystemContext, errorJson, id, json, query, transaction } from "./db";
import { assertSubscriptionActive, limitReached, subscriptionUsage } from "./limits";
...
export async function acceptTeamInvitation(request, context) {
  try {
    ...
    assertCsrf(request);
    await enforceRequestRateLimit(request, invitationHash(params.token), 'password');
    enterSystemContext();
    const body = await readJsonBodyLimited(request, 16384);
    const name = clean(body.name);
    const password = String(body.password || "");
    if (!name || password.length < 12) throw new AppError("Name and a password of at least 12 characters are required.", 400, "VALIDATION_ERROR");
    const session = await transaction(async (client) => {
      ...
      await client.query("SELECT id FROM businesses WHERE id = $1 FOR UPDATE", [invitation.business_id]);
      const plan = await subscriptionUsage(invitation.business_id, client);
      assertSubscriptionActive(plan);
      const capacityError = limitReached("User", plan.limits.users, plan.usage.users);
      ...
      return { userId: user.id, businessId: invitation.business_id, role: invitation.role, sessionVersion: user.session_version || 0 };
    });
