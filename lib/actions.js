    ...
    const result = await client.query(
      `SELECT j.id AS job_id, j.attempts, j.max_attempts, j.campaign_recipient_id,
       ...
       WHERE c.status NOT IN ('paused', 'cancelled') AND j.status IN ('queued', 'retry') AND j.run_at <= NOW()
         ${process.env.SUBSCRIPTION_ENFORCEMENT_ENABLED === 'true' ? "AND (b.review_access = TRUE OR (bs.status = 'active' AND (bs.current_period_end IS NULL OR bs.current_period_end > NOW())) OR (bs.status = 'trialing' AND bs.trial_ends_at > NOW()))" : ""}
         ${businessFilter}
