...
import { createWhatsAppTemplate, decryptSecret, encryptSecret, fetchWhatsAppMedia, listWhatsAppTemplates, metaReady, sendInteractiveMessage, sendTemplateMessage, sendTextMessage, templateApiName } from "./meta";
    ...
    await transaction(async (client) => {
        ...
        const account = await client.query(
          `INSERT INTO whatsapp_accounts (id,business_id,waba_id,onboarding_method,access_token_encrypted,is_default,status,webhook_subscribed,token_expires_at,last_synced_at)
           ...
           ON CONFLICT (business_id,waba_id) DO UPDATE SET access_token_encrypted=EXCLUDED.access_token_encrypted,is_default=TRUE,status='connected',webhook_subscribed=TRUE,token_expires_at=EXCLUDED.token_expires_at,updated_at=NOW()
           RETURNING id`,
...
async function claimCampaignJobs({ businessId = "", limit }) {
  return transaction(async (client) => {
    ...
    const stale = await client.query(
      `SELECT j.id, j.campaign_recipient_id FROM campaign_jobs j
...
const WORKSPACE_VIEWS = new Set(["overview", "setup", "contacts", "team", "billing", "security", "templates", "automation", "campaigns", "results", "inbox", "unsubscribes"]);
...
async function messagingSetupForContact(businessId, contactId) {
  const result = await query(
    `SELECT b.*, c.whatsapp_phone_number_id AS conversation_phone_number_id,
...
function webhookInputFromMessage(message) {
  ...
  return {
    ...
    caption,
    metadata: { filename: clean(media.filename), sha256: clean(media.sha256), voice: Boolean(media.voice), ...(referral ? { referral } : {}) }
  };
}
