...
export async function importContacts(request) {
  try {
    ...
    for (const cells of rows) {
      ...
      const allowed = permissionFromCell(third || "yes");
      await query(
        `INSERT INTO contacts (id, business_id, name, phone, marketing_permission, unsubscribed, source)
    ...
    if (!imported) throw new AppError("No valid contacts found. Use columns: name, phone, permission.", 400, "CSV_EMPTY");
    await audit(session, "contacts_imported", { imported, skipped });
    const nextState = await loadState(await currentAccount(request));
    return json({ ...nextState, importSummary: { imported, skipped } }, 201);
  } catch (error) {
    return errorJson(error);
  }
}
...
export async function createCampaign(request) {
  try {
    ...
    await transaction(async (client) => {
      ...
      await client.query("INSERT INTO events (id, business_id, type, metadata) VALUES ($1, $2, 'campaign_queued', $3)", [id("e"), session.businessId, JSON.stringify({ recipients: contactResult.rows.length })]);
    });
    await audit(session, "campaign_queued", { contacts: contactResult.rows.length });
    return json(await loadState(await currentAccount(request)), 201);
  } catch (error) {
    return errorJson(error);
  }
...
async function loadState(account) {
  ...
  const [business, contacts, templates, campaigns, recipients, conversations, messages, events, automationFlows] = await Promise.all([
