...
export async function importContacts(request) {
  try {
    ...
    for (const cells of rows) {
      const [first = "", second = "", third = ""] = cells.map((cell) => clean(cell));
      ...
      validRows.push({ name: rawName, phone, allowed: permissionFromCell(third || "yes") });
    }
    ...
    for (const row of validRows) {
      await query(
        ...
        [id("c"), session.businessId, row.name, row.phone, row.allowed, !row.allowed]
      );
    }
...
export async function createCampaign(request) {
  try {
    ...
    await transaction(async (client) => {
      ...
      await client.query(
        "INSERT INTO campaigns (id, business_id, name, template_id, automation_flow_id, variables, mode) VALUES ($1, $2, $3, $4, $5, $6, $7)",
        [campaignId, session.businessId, clean(body.name) || "Untitled Campaign", template.id, linkedFlow?.id || null, JSON.stringify(variables), business.mode]
      );
  ...
  return transaction(async (client) => {
    ...
    const result = await client.query(
      `SELECT j.id AS job_id, j.attempts, j.max_attempts, j.campaign_recipient_id,
              ...
              t.variables AS template_variables, t.meta_template_name,
