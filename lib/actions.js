...
import { encryptSecret, sendTemplateMessage } from "./meta";
...
export async function updateSetup(request) {
  try {
    ...
    const mode = body.mode === "Live Meta" ? "Live Meta" : "Mock Meta";
    ...
    const status = phoneNumberId && wabaId && (mode === "Mock Meta" || nextToken) ? "Connected" : "Needs setup";
...
export async function createTemplate(request) {
  try {
    ...
    await query(
      `INSERT INTO templates (id, business_id, name, body, variables, status, meta_template_id, source)
       VALUES ($1, $2, $3, $4, $5, $6, $7, $8)`,
      [id("t"), session.businessId, name, templateBody, JSON.stringify(templateVariables(templateBody)), submitToMeta ? "Pending" : "Draft", submitToMeta ? id("meta_tpl") : "", submitToMeta ? "Pending Meta submission" : "Manual draft"]
    );
...
export async function patchTemplate(request, context) {
  try {
    const session = await requireSession(request);
    const params = await context.params;
    const body = await request.json();
    if (!["Draft", "Pending", "Approved", "Rejected"].includes(body.status)) throw new AppError("Invalid template status.", 400, "VALIDATION_ERROR");
    await query("UPDATE templates SET status = $1, updated_at = NOW() WHERE id = $2 AND business_id = $3", [body.status, params.id, session.businessId]);
    await audit(session, "template_status_updated", { templateId: params.id, status: body.status });
    return json(await loadState(await currentAccount(request)));
  } catch (error) {
    return errorJson(error);
  }
...
export async function sendReply(request) {
  try {
    ...
    await query("INSERT INTO messages (id, conversation_id, direction, body, status) VALUES ($1, $2, 'outgoing', $3, 'sent')", [id("m"), conversationId, clean(body.body)]);
...
export async function sendTemplateReply(request) {
  try {
    ...
    const conversationId = await findOrCreateConversation(session.businessId, contact.id);
    await query("INSERT INTO messages (id, conversation_id, direction, body, status) VALUES ($1, $2, 'outgoing', $3, 'sent')", [id("m"), conversationId, message]);
...
export async function mockIncoming(request) {
  try {
    if (process.env.ENABLE_MOCK_META === "false") throw new AppError("Mock Meta webhook is disabled.", 404, "NOT_FOUND");
    const session = await requireSession(request);
    const body = await request.json();
    const contact = await findContact(session.businessId, body.contactId);
    if (!contact) throw new AppError("Contact not found.", 404, "NOT_FOUND");
    await ingestIncoming(session.businessId, contact.phone, clean(body.body) || "Hello", new Date());
    return json(await loadState(await currentAccount(request)), 201);
  } catch (error) {
    return errorJson(error);
  }
...
export async function receiveMetaWebhook(request) {
  try {
    ...
    for (const change of changes) {
      ...
      const business = await businessByPhoneNumberId(value.metadata?.phone_number_id);
...
export async function resetData(request) {
  try {
    if (process.env.ENABLE_ADMIN_RESET !== "true") throw new AppError("Admin reset is disabled.", 403, "RESET_DISABLED");
    const session = await requireSession(request);
    await transaction(async (client) => {
      await client.query("DELETE FROM events WHERE business_id = $1", [session.businessId]);
      await client.query("DELETE FROM campaigns WHERE business_id = $1", [session.businessId]);
      await client.query("DELETE FROM conversations WHERE business_id = $1", [session.businessId]);
      await client.query("DELETE FROM templates WHERE business_id = $1", [session.businessId]);
      await client.query("DELETE FROM contacts WHERE business_id = $1", [session.businessId]);
    });
    return json(await loadState(await currentAccount(request)));
  } catch (error) {
    return errorJson(error);
...
async function loadState(account) {
  ...
  return {
    ...
    meta: {
      ...
      mockMetaEnabled: process.env.ENABLE_MOCK_META !== "false"
    }
  };
}
...
async function businessByPhoneNumberId(phoneNumberId) {
  if (!phoneNumberId) return null;
  return (await query("SELECT * FROM businesses WHERE phone_number_id = $1", [String(phoneNumberId)])).rows[0] || null;
}
