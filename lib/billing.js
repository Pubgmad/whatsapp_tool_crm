...
export async function createCheckout(request) {
  try {
    ...
    const trialDays = !subscription?.provider_subscription_id ? Number(plan.trial_days) : 0;
    ...
    }, { idempotencyKey: 'checkout:' + session.businessId + ':' + plan.id + ':' + interval + ':' + Math.floor(Date.now() / 3600000) });
...
export async function receiveStripeWebhook(request) {
  try {
    ...
    if (rawBody.length > 256000) throw new AppError('Webhook payload is too large.', 413, 'WEBHOOK_TOO_LARGE');
    const stripe = stripeClient();
    let event;
