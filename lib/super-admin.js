...
export function superSessionCookie(token) {
  ...
  return `${SUPER_SESSION_COOKIE_NAME}=${encodeURIComponent(token)}; Path=/; HttpOnly; SameSite=Lax; Max-Age=${SUPER_SESSION_SECONDS}${secure}`;
}
...
export async function requireSuperAdmin(request) {
  return authenticateSuperAdminToken(parseCookies(request)[SUPER_SESSION_COOKIE_NAME]);
}
