...
function Screens({ activeView, ...props }) {
  const screens = { overview: <Overview {...props} />, setup: <Setup {...props} />, contacts: <Contacts {...props} />, team: <Team {...props} />, templates: <Templates {...props} />, automation: <AutomationFlows {...props} />, campaigns: <Campaigns {...props} />, results: <Results {...props} />, inbox: <InboxView {...props} />, unsubscribes: <Unsubscribes {...props} /> };
  return screens[activeView];
}

function pageTitle(view) {
  return { overview: "Command center", setup: "Business connection", contacts: "Audience", team: "Team workspace", templates: "Template library", automation: "Automation flows", campaigns: "Campaign builder", results: "Campaign results", inbox: "Inbox", unsubscribes: "Suppression list" }[view];
}
...
function Templates({ state, mutate }) {
  const create = (event) => {
    ...
    mutate(postJson("/api/templates", { name: values.get("name"), category: values.get("category"), language: values.get("language"), headerText: values.get("headerText"), body: values.get("body"), footerText: values.get("footerText"), buttons: values.get("buttons"), submitToMeta: true }).then((next) => { form.reset(); return next; }), "Template submitted");
  };
  const sync = () => mutate(postJson("/api/templates/sync", {}), "Templates synced");
  return <div className="screenGrid"><section className="actionBand"><div><strong>Meta template library</strong><span>Approval status and content stay synchronized with your WABA.</span></div><button className="secondaryAction" type="button" onClick={sync}><RefreshCcw size={18} /> Sync from Meta</button></section><Panel title="Create WhatsApp template" subtitle="Submit a body with optional text header, footer, and quick replies"><form className="templateComposer fullTemplateComposer" onSubmit={create}><Input name="name" label="Template name" required /><label>Category<select name="category"><option value="MARKETING">Marketing</option><option value="UTILITY">Utility</option><option value="AUTHENTICATION">Authentication</option></select></label><label>Language<select name="language"><option value="en_US">English (US)</option><option value="en">English</option><option value="en_GB">English (UK)</option><option value="hi">Hindi</option></select></label><Input name="headerText" label="Text header" maxLength="60" /><label className="templateBodyField">Body<textarea name="body" placeholder="Use variables like {{name}}" required /></label><Input name="footerText" label="Footer" maxLength="60" /><Input name="buttons" label="Quick replies" placeholder="Pricing, Book demo, Talk to agent" /><button className="primaryAction" type="submit"><MessageSquareText size={18} /> Submit to Meta</button></form></Panel><div className="cardGrid">{state.templates.map((template) => <article className="templateCard" key={template.id}><div className="cardHead"><div><h3>{template.name}</h3><small>{template.category} | {template.language}</small></div><Badge kind={template.status === "Approved" ? "good" : template.status === "Pending" ? "warn" : "bad"}>{template.status}</Badge></div>{template.headerText && <strong className="templateHeaderText">{template.headerText}</strong>}<p>{template.body}</p>{template.footerText && <small className="templateFooterText">{template.footerText}</small>}<div className="chipRow">{(template.buttons || []).map((button) => <span key={button.text}>{button.text}</span>)}{template.variables.map((variable) => <span key={variable}>{`{{${variable}}}`}</span>)}</div>{template.rejectionReason && <small className="errorLine">{template.rejectionReason}</small>}</article>)}</div>{!state.templates.length && <Panel title="No templates"><EmptyState text="Submit or sync a WhatsApp template to begin" /></Panel>}</div>;
}
...
function InboxView({ state, activeConversation, activeContact, approvedTemplates, setActiveConversationId, mutate }) {
  ...
  const selectConversation = (conversation) => { setActiveConversationId(conversation.id); if (conversation.unreadCount > 0) mutate(postJson(`/api/conversations/${conversation.id}/workflow`, { action: "mark_read" }, "PATCH")); };
