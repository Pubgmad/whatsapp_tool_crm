...
import {
  ...
  ShieldCheck, Sparkles, Trash2, Upload, UsersRound, X, Search, Clock3
} from "lucide-react";
    ...
    <main className="shell">
      <aside className={`sideRail ${mobileNavOpen ? "open" : ""}`}>
        ...
        <nav className="navList" aria-label="Product sections">
          {navItems.map((item) => { const Icon = item.icon; return <button key={item.id} className={activeView === item.id ? "active" : ""} onClick={() => { setActiveView(item.id); setMobileNavOpen(false); }}><Icon size={18} /><span>{item.label}</span></button>; })}
        </nav>
...
function Contacts({ state, mutate }) {
  ...
  const filteredContacts = state.contacts.filter((contact) => {
    const matchesSearch = !search || [contact.name, contact.phone, ...(contact.tags || [])].join(" ").toLowerCase().includes(search.toLowerCase());
    const matchesPermission = permissionFilter === "all" || (permissionFilter === "allowed" ? contact.marketingPermission && !contact.unsubscribed : contact.unsubscribed || !contact.marketingPermission);
  ...
  return <div className="screenGrid"><div className="contentGrid audienceGrid"><Panel title="Add contact" subtitle="Create one opted-in customer record"><form className="formGrid" onSubmit={add}><Input name="name" label="Name" placeholder="Customer name" required /><Input name="phone" label="Phone" placeholder="Phone with country code" required /><Input name="tags" label="Tags" placeholder="lead, customer, priority" /><Input name="optInSource" label="Opt-in source" placeholder="Website form, event, store" /><label className="checkRow"><input name="marketingPermission" type="checkbox" /> Marketing permission</label><button className="primaryAction" type="submit"><Plus size={18} /> Add contact</button></form></Panel><Panel title="Import contacts" subtitle="Upload or paste CSV columns: name, phone, permission, tags"><form className="formGrid importForm" onSubmit={importRows}><label className="fileDrop"><input type="file" accept=".csv,text/csv" onChange={chooseFile} /><Upload size={22} /><strong>{fileName || "Choose CSV file"}</strong><span>{fileName ? "File loaded into the import box" : "You can also paste rows below"}</span></label><label>CSV rows<textarea name="csv" value={csvText} onChange={(event) => setCsvText(event.target.value)} placeholder={"name,phone,permission,tags"}></textarea></label><div className="importMeta"><span>{csvText.trim() ? `${csvText.trim().split(/\r?\n/).length} row${csvText.trim().split(/\r?\n/).length === 1 ? "" : "s"} ready` : "No rows loaded"}</span><button className="secondaryAction" type="submit" disabled={!csvText.trim()}><Upload size={18} /> Import contacts</button></div></form></Panel></div><Panel title="Contacts" subtitle={`${filteredContacts.length} shown of ${state.contacts.length}`}><div className="dataToolbar"><label className="searchBox"><Search size={17} /><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Search name, phone or tag" /></label><label className="filterBox"><select value={permissionFilter} onChange={(event) => setPermissionFilter(event.target.value)}><option value="all">All contacts</option><option value="allowed">Marketable</option><option value="blocked">Suppressed</option></select></label></div><DataTable headers={["Name", "Phone", "Permission", "Last message", "Actions"]}>{filteredContacts.map((contact) => <tr key={contact.id}><td><strong>{contact.name}</strong></td><td><div>{contact.phone}</div><div className="chipRow compact">{(contact.tags || []).map((tag) => <span key={tag}>{tag}</span>)}</div></td><td><Badge kind={contact.unsubscribed ? "bad" : contact.marketingPermission ? "good" : "bad"}>{contact.unsubscribed ? "Suppressed" : contact.marketingPermission ? "Allowed" : "Blocked"}</Badge></td><td>{formatTime(contact.lastMessageAt)}</td><td className="rowActions"><button onClick={() => mutate(postJson(`/api/contacts/${contact.id}`, { marketingPermission: !contact.marketingPermission, unsubscribed: contact.marketingPermission }, "PATCH"), "Updated")}>{contact.marketingPermission ? "Suppress" : "Allow"}</button><button className="dangerText" onClick={() => mutate(postJson(`/api/contacts/${contact.id}`, {}, "DELETE"), "Removed")}>Remove</button></td></tr>)}</DataTable></Panel></div>;
}

function Templates({ state, mutate }) {
  const create = (event) => { event.preventDefault(); const form = new FormData(event.currentTarget); mutate(postJson("/api/templates", { name: form.get("name"), category: form.get("category"), language: form.get("language"), body: form.get("body"), submitToMeta: true }), "Template submitted"); event.currentTarget.reset(); };
  const sync = () => mutate(postJson("/api/templates/sync", {}), "Templates synced");
  return <div className="screenGrid"><section className="actionBand"><button className="secondaryAction" type="button" onClick={sync}><RefreshCcw size={18} /> Sync from Meta</button></section><Panel title="Create template"><form className="templateComposer richTemplateComposer" onSubmit={create}><Input name="name" label="Name" placeholder="Template name" /><label>Category<select name="category"><option value="MARKETING">Marketing</option><option value="UTILITY">Utility</option><option value="AUTHENTICATION">Authentication</option></select></label><label>Language<select name="language"><option value="en_US">English (US)</option><option value="en">English</option><option value="en_GB">English (UK)</option><option value="hi">Hindi</option></select></label><label className="templateBodyField">Body<textarea name="body" placeholder="Use variables like {{name}} where needed"></textarea></label><button className="primaryAction" type="submit"><MessageSquareText size={18} /> Submit</button></form></Panel><div className="cardGrid">{state.templates.map((template) => <article className="templateCard" key={template.id}><div className="cardHead"><div><h3>{template.name}</h3><small>{template.category} · {template.language}</small></div><Badge kind={template.status === "Approved" ? "good" : template.status === "Pending" ? "warn" : "bad"}>{template.status}</Badge></div><p>{template.body}</p><div className="chipRow">{template.variables.map((variable) => <span key={variable}>{`{{${variable}}}`}</span>)}</div></article>)}</div></div>;
}
...
function Campaigns({ state, approvedTemplates, marketableContacts, mutate, setActiveView }) {
  ...
  const submit = async (event) => {
    ...
    try {
      await postJson("/api/campaigns", { name: form.get("name"), templateId: form.get("templateId"), automationFlowId, variables, contactIds: form.getAll("contactIds"), scheduledAt: form.get("scheduledAt") ? new Date(form.get("scheduledAt")).toISOString() : "", timezone: Intl.DateTimeFormat().resolvedOptions().timeZone });
  ...
  return <div className="campaignLayout"><Panel title="Campaign"><form className="formGrid" onSubmit={submit}><Input name="name" label="Name" placeholder="Campaign name" /><label>Template<select name="templateId" value={template?.id || ""} onChange={(event) => { setTemplateId(event.target.value); setVariables({}); }} disabled={!approvedTemplates.length}><option value="">{approvedTemplates.length ? "Choose template" : "No approved templates available"}</option>{approvedTemplates.map((item) => <option value={item.id} key={item.id}>{item.name}</option>)}</select></label><label>Schedule<input name="scheduledAt" type="datetime-local" /></label><label>Reply automation<select value={automationFlowId} onChange={(event) => setAutomationFlowId(event.target.value)}><option value="">No follow-up flow</option>{activeFlows.map((flow) => <option key={flow.id} value={flow.id}>{flow.name}</option>)}</select></label>{!approvedTemplates.length && <EmptyState text="Create a template and wait for Meta approval before sending campaigns" />}{editableVariables.map((variable) => <label key={variable}>{variable}<input value={variables[variable] || ""} onChange={(event) => setVariable(variable, event.target.value)} placeholder={`Value for {{${variable}}}`} /></label>)}<label className="searchBox recipientSearch"><Search size={17} /><input value={recipientSearch} onChange={(event) => setRecipientSearch(event.target.value)} placeholder="Search eligible contacts" /></label><div className="recipientBox">{filteredRecipients.map((contact) => <label key={contact.id}><span>{contact.name}<small>{contact.phone}</small></span><input name="contactIds" value={contact.id} type="checkbox" /></label>)}{!marketableContacts.length && <EmptyState text="No eligible contacts" />}</div><button className="primaryAction" type="submit" disabled={!approvedTemplates.length || !marketableContacts.length}><Clock3 size={18} /> Queue campaign</button></form></Panel><Panel title="WhatsApp preview"><div className="phonePreview"><div className="waBubble">{preview}</div></div></Panel></div>;
}
function Results({ state, mutate }) {
  const processQueue = () => mutate(postJson("/api/campaigns/process", { limit: 25 }), "Queue processed");
  return <div className="screenGrid"><section className="actionBand"><button className="secondaryAction" type="button" onClick={processQueue}><RefreshCcw size={18} /> Process queue</button></section>{state.campaigns.map((campaign) => <Panel key={campaign.id} title={campaign.name} subtitle={campaign.scheduledAt ? `Scheduled ${formatTime(campaign.scheduledAt)} · ${campaign.timezone}` : formatTime(campaign.createdAt)}><div className="resultHeader"><Badge kind={campaign.status === "failed" ? "bad" : campaign.status === "scheduled" || campaign.status === "processing" ? "warn" : "good"}>{campaign.status}</Badge></div><ResultMeters stats={campaign.stats} /><DataTable headers={["Customer", "Status", "Message"]}>{campaign.recipients.map((recipient) => { const contact = state.contacts.find((item) => item.id === recipient.contactId) || {}; return <tr key={recipient.id || recipient.metaMessageId}><td><strong>{contact.name || "Unknown"}</strong></td><td><Badge kind={recipient.status === "failed" ? "bad" : recipient.status === "queued" ? "warn" : "good"}>{recipient.status}</Badge></td><td><span>{recipient.message}</span>{recipient.errorMessage && <small className="errorLine">{recipient.errorMessage}</small>}</td></tr>; })}</DataTable></Panel>)}{!state.campaigns.length && <Panel title="No results"><EmptyState text="No campaigns yet" /></Panel>}</div>;
}
...
function InboxView({ state, activeConversation, activeContact, approvedTemplates, setActiveConversationId, mutate }) {
  ...
  const filteredConversations = state.conversations.filter((conversation) => {
    ...
    const matchesFilter = inboxFilter === "all" || (inboxFilter === "human" ? conversation.automationPaused : inboxFilter === "unassigned" ? !conversation.assignedUserId : conversation.canReply);
